...
class TestCompf:
    ...
    def test_expressions7(self):
        assert self.c.compile("c+(c+(c*(c+(c/(c*(c+c))))))") == \
            "c c c c c c c c + * / + * + +"
    def test_fig_expressions1(self):
        assert self.c.compile('{a+b}') == 'a b + D *'

    def test_fig_one_symbol(self):
        assert self.c.compile('{a}') == 'a D *'

    def test_three_symbol_fig(self):
        assert self.c.compile('{a+b+c}') == 'a b + c + D *'

    def test_double_caps_fig(self):
        assert self.c.compile('{{a+b}}') == 'a b + D * D *'

    def test_hard(self):
        assert self.c.compile('{a}+{b}') == 'a D * b D * +'

    def test_super_hard(self):
        assert self.c.compile('a+{a+{a+{a}}}') == 'a a a a D * + D * + D * +'

    def test_figure_ultimate(self):
        assert self.c.compile('({a}+{b+c})+{d+e+f}') == 'a D * b c + D * + d e + f + D * +'